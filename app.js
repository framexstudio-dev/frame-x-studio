...
function applySectionOrder(settings){const base=['sobre','servicos','projetos','videos','imagens','restauracao','modelos','processo','visao','stats','avaliacoes','contato'];const order=[...(settings.sectionOrder||base).filter(k=>base.includes(k)),...base.filter(k=>!(settings.sectionOrder||[]).includes(k))];const main=document.querySelector('main');const anchor=document.querySelector('.marquee');for(const key of order){const el=document.querySelector(`[data-section-key="${key}"]`);if(el)main.appendChild(el)}const hidden=new Set(settings.hiddenSections||[]);base.forEach(k=>{const el=document.querySelector(`[data-section-key="${k}"]`);if(el)el.style.display=hidden.has(k)?'none':''})}
...
async function renderImages(s){
 const list=s.images.filter(v=>v.published!==false&&v.status!=='draft').sort((a,b)=>(Number(b.featured)-Number(a.featured))||((a.order||0)-(b.order||0)));$('#emptyImages').style.display=list.length?'none':'block';if(!list.length){$('#imageTrack').innerHTML='';return}imageIndex=Math.min(imageIndex,list.length-1);const html=[];for(const v of list){const src=await assetSrc(v),ratio=v.ratio||'horizontal';html.push(`<button class="coverItem ratio-${esc(ratio)}" data-src="${esc(src)}" data-title="${esc(v.title||'Imagem')}" data-desc="${esc(v.description||'')}"><img src="${esc(src)}" alt="${esc(v.title||'Imagem')}"><div class="coverMeta"><small>${esc(v.category||'IMAGEM')}</small><b>${esc(v.title||'Imagem')}</b></div></button>`)}$('#imageTrack').innerHTML=html.join('');setupCoverflow('image',list.length);$$('#imageTrack .coverItem').forEach(el=>el.onclick=()=>openImage(el.dataset.src,el.dataset.title,el.dataset.desc))
}
